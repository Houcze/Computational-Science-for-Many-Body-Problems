...
# # Exercise code for Monte Calro simulation of the 3d Ising model
# This module is for simulation of 3d Ising model on the cubic lattice, whose Hamiltonian is given by
...
# 2023 May Tsuyoshi Okubo, remove wo_jit version, added improved estimator for M^4
# 2023 June HOU Chengze, implemented for cubic lattice.
...
def metropolis(S, exps, L):
    N = L**3
    ran = np.random.rand(N).reshape(L, L, L)
    for ix in range(L):
        for iy in range(L):
            for iz in range(L):
                hm = (S[ix, iy, iz]*(S[(ix + 1) % L, iy, iz] + S[ix, (iy + 1) % L, iz] + S[ix, iy,
                      (iz + 1) % L] + S[(ix - 1 + L) % L, iy, iz] + S[ix, (iy - 1 + L) % L, iz] + S[ix, iy, (iz- 1 + L) % L]) + 6)//2
                if ran[ix, iy, iz] < exps[hm, (S[ix, iy, iz]+1)//2]:
                    S[ix, iy, iz] *= -1
    return S
...
def heatbath(S, exps, L):
    ...
    for ix in range(L):
        for iy in range(L):
            for iz in range(L):
                hm = ((S[(ix + 1) % L, iy, iz] + S[ix, (iy + 1) % L, iz] + S[ix, iy, (iz + 1) % L] +
                      S[(ix - 1 + L) % L, iy, iz] + S[ix, (iy - 1 + L) % L, iz] + S[ix, iy, (iz - 1 + L) % L]) + 6)//2
                if ran[ix, iy, iz] < exps[hm, (S[ix, iy, iz]+1)//2]:
                    S[ix, iy, iz] = 1
                else:
                    S[ix, iy, iz] = -1
...
def make_bond(S, prob, L):
    N = L**3
    bond = np.zeros((L, L, L, 3), dtype=np.int64)
    ran = np.random.rand(3 * N).reshape(L, L, L, 3)
    for ix in range(L):
        for iy in range(L):
            for iz in range(L):
                if S[ix, iy, iz] * S[(ix + 1) % L, iy, iz] > 0:  # spins are parallel along x
                    if ran[ix, iy, iz, 0] < prob:
                        bond[ix, iy, iz, 0] = 1
                if S[ix, iy, iz] * S[ix, (iy + 1) % L, iz] > 0:  # spins are parallel along y
                    if ran[ix, iy, iz, 1] < prob:
                        bond[ix, iy, iz, 1] = 1
                if S[ix, iy, iz] * S[ix, iy, (iz + 1) % L] > 0:  # spins are parallel along z
                    if ran[ix, iy, iz, 2] < prob:
                        bond[ix, iy, iz, 2] = 1
    return bond
...
def make_cluster(bond, L):
    ...
    for ix in range(L):
        for iy in range(L):
            for iz in range(L):
                if bond[ix, iy, iz, 0] > 0:  # connected along x
                    ni = ix + iy * L + iz * L**2
                    nj = (ix + 1) % L + iy * L + iz * L**2
                    update_cluster_num(ni, nj, cluster_num)
                if bond[ix, iy, iz, 1] > 0:  # connected along y
                    ni = ix + iy * L + iz * L**2
                    nj = ix + (iy + 1) % L * L + iz * L**2
                    update_cluster_num(ni, nj, cluster_num)
                if bond[ix, iy, iz, 2] > 0:  # connected along z
                    ni = ix + iy * L + iz * L**2
                    nj = ix + iy * L + (iz + 1) % L * L**2
                    update_cluster_num(ni, nj, cluster_num)
    ...
    return cluster_num.reshape(L, L, L), true_cluster_num_count[:total_cluster_num]
...
def flip_spin(S, cluster_num, cluster_num_count, flip, L):
    ...
    for ix in range(L):
        for iy in range(L):
            for iz in range(L):
                S[ix, iy, iz] = spin_direction[cluster_num[ix, iy, iz]]


def Swendsen_Wang(S, prob, flip, L):
    N = L**3

    # make bond configurations
...
def Calc_local_energy(S, L):
    ...
    for ix in range(L):
        for iy in range(L):
            for iz in range(L):
                local_ene += S[ix, iy, iz] * (S[(ix + 1) % L, iy, iz] +
                                              S[ix, (iy + 1) % L, iz] + S[ix, iy, (iz+1) % L])
    return local_ene
...
def Initialize(seed, L):
    ...
    return np.random.choice(mp, N).reshape(L, L, L)
...
def MC(L, T, h, thermalization, observation, seed=11, algorithm="metropolis", output_snapshots=False):
    N = L**3
    S = Initialize(seed, L)
    exps = np.zeros((7, 2)) # for heatbath and metropolis
    ...
    for i in range(thermalization):
        if algorithm == "heatbath":
            ...
            S, m2_imp, m4_imp = Swendsen_Wang(S, prob, flip, L)
        else:
    ...
    mag4 = np.array(mag4)
    mag4_imp = np.array(mag4_imp)
    mag_abs = np.array(mag_abs)
    if output_snapshots:
        return mag, mag2, mag2_imp, mag4, mag4_imp, mag_abs, ene, ene2, S
    else:
        return mag, mag2, mag2_imp, mag4, mag4_imp, mag_abs, ene, ene2
...
def Jackknife(data, bin_size=100, func=None, data2=None):
    def make_bin(data, bin_size_in):
        ...
        for i in range(bin_num):
            bin_data_temp[i] = np.sum(
                data[i * bin_size: (i+1)*bin_size]) / bin_size
    ...
    if func is not None:
        if data2 is not None:
            ...
            error = np.sqrt((np.sum(func(bin_data, bin_data2)**2) /
                            len(bin_data) - average ** 2) * (len(bin_data)-1))

        else:
            # we assume that func is a function of data
            bin_data, bin_size_out = make_bin(data, bin_size)
            average = np.sum(func(bin_data))/(len(bin_data))
            error = np.sqrt((np.sum(func(bin_data)**2) /
                            len(bin_data) - average ** 2) * (len(bin_data)-1))
        ...
        error = np.sqrt((np.sum(bin_data**2)/len(bin_data) -
                        average ** 2) * (len(bin_data)-1))
...
def main():
    ...
    def parse_args():
        ...
        parser = argparse.ArgumentParser(
            description='Monte Carlo simulation of the square lattice Ising model')
    ...
    # run MC simulation
    mag, mag2, mag2_imp, mag4, mag4_imp, mag_abs, ene, ene2 = MC(
        L, T, h, thermalization, observation, random_seed, algorithm)
    ...
    C, C_err = Jackknife(ene, bin_size=max(
        100, observation//100), func=variance, data2=ene2)
    C *= N/T**2
    C_err *= N/T**2
    b, b_err = Jackknife(mag2, bin_size=max(
        100, observation//100), func=binder, data2=mag4)
    chi, chi_err = Jackknife(mag_abs, bin_size=max(
        100, observation//100), func=variance, data2=mag2)
    ...
    print("Another Susceptibility = " +
          repr(chi/T * N) + " +- " + repr(chi_err/T * N))
    print("Binder ratio = " + repr(b) + " +- " + repr(b_err))

    if algorithm == "cluster":
        M2_imp, M2_imp_err = Jackknife(
            mag2_imp, bin_size=max(100, observation/100))
        print("Magnetization^2: improved estimator = " +
              repr(M2_imp) + " +- " + repr(M2_imp_err))
        print("Susceptibility: improved estimator  = " +
              repr(M2_imp/T * N) + " +- " + repr(M2_imp_err/T * N))
        b_imp, b_imp_err = Jackknife(mag2_imp, bin_size=max(
            100, observation//100), func=binder, data2=mag4_imp)
        print("Binder ratio: improved estimator = " +
              repr(b) + " +- " + repr(b_err))
