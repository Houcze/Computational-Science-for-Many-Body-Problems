...
# Finite size scaling for Monte Calro simulation of the 2d Ising model
...
def parse_args():
    ...
    parser.add_argument('--L_list',metavar='L_list',dest='L_list', type=int, nargs='+', default=[32, 64, 128],
                        help='the list of sizes of square lattice. (default: L_list = [32, 64, 128, 256, 512])')
...
def Calc_averages(mag, mag2, mag2_imp, mag4, mag4_imp, mag_abs, ene,ene2, L,T,alogorithm):
    ...
    if algorithm == "cluster":
        ...
        b_imp = 0.0
        b_imp_err = 0.0
    return e, e_err, m, m_err, m2, m2_err, c, c_err, b,b_err,chi, chi_err,m2_imp, m2_imp_err,b_imp, b_imp_err
...
if Read_simulation_data:
    ...
    for i in range(len(L_list)):
        ...
        for j in range(len(T_list_all[i])):
            ...
            e, e_err, m, m_err, m2, m2_err, c, c_err, b,b_err,chi, chi_err,m2_imp, m2_imp_err,b_imp, b_imp_err = Calc_averages(mag, mag2, mag2_imp, mag4, mag4_imp, mag_abs, ene,ene2, L,T,algorithm)
    ...
    f = open(data_file,"wb")
    obs_list_all = [Ene_all,Ene_err_all,Mag_all,Mag_err_all,Mag2_all,Mag2_err_all,Mag2_imp_all,Mag2_imp_err_all,Binder_all,Binder_err_all,Binder_imp_all,Binder_imp_err_all,C_all,C_err_all,Chi_all,Chi_err_all,L_list,T_list_all]
...
pyplot.legend()
pyplot.savefig('obs_cluster/Energe.jpg')
pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Specific heat.jpg')
pyplot.clf()
...
pyplot.xlabel("$T$")
pyplot.ylabel("$C$")
Tc_index = np.argmax(C_all[-1]) # 找到最后一个L对应的C_all中的最大值的索引
T_C = T_list_all[-1][Tc_index]  # 找到对应的温度
for i in range(len(L_list)):
    pyplot.errorbar(T_list_all[i],C_all[i],yerr=np.array(C_err_all[i]),capsize=3,fmt="o",label = "L="+repr(L_list[i]))
pyplot.axvline(x=T_C, color='r', linestyle='--')
pyplot.text(T_C, 1.05*np.max(C_all[-1]), '$T_C$', color = 'red')
pyplot.xticks([T_C], ['$T_C: 2.275$'])
for label in pyplot.gca().get_xticklabels():
    label.set_rotation(45)
pyplot.legend()
pyplot.savefig('obs_cluster/Specific heat wiht Tc.jpg')
pyplot.clf()
...
pyplot.ylabel("$M$")
for i in range(len(L_list)):
    pyplot.errorbar(T_list_all[i],Mag_all[i],yerr=np.array(Mag_err_all[i]),capsize=3,fmt="o",label = "L="+repr(L_list[i]))
pyplot.axvline(x=2.295, color='r', linestyle='--')
pyplot.text(2.295, 1.05*np.max(C_all[-1]), '$T_C$', color = 'red')
pyplot.xticks([2.295], ['$T_C: 2.295$'])
for label in pyplot.gca().get_xticklabels():
    label.set_rotation(45)
pyplot.legend()
pyplot.savefig('obs_cluster/Magnetization with Tc.jpg')
pyplot.clf()
...
for i in range(len(L_list)):
    pyplot.errorbar(T_list_all[i],Mag_all[i],yerr=np.array(Mag_err_all[i]),capsize=3,fmt="o",label = "L="+repr(L_list[i]))
pyplot.legend()
pyplot.savefig('obs_cluster/Magnetization.jpg')
pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Squared magnetization.jpg')
pyplot.clf()
...
if algorithm == "cluster":
    ...
    pyplot.legend()
    pyplot.savefig('obs_cluster/Squared magnetization improved.jpg')
    pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Binder ratio.jpg')
pyplot.clf()
...
for i in range(len(L_list)):
    pyplot.errorbar(T_list_all[i],Binder_all[i],yerr=np.array(Binder_err_all[i]),capsize=3,fmt="o",label = "L="+repr(L_list[i]))
pyplot.legend()
min_variance_index = np.argmin(np.var(Binder_all, axis=0))
min_variance_temperature = T_list_all[0][min_variance_index] 
pyplot.axvline(x=min_variance_temperature, color='r', linestyle='--')
pyplot.xticks([min_variance_temperature], ['$T_C: 2.265$'])
for label in pyplot.gca().get_xticklabels():
    label.set_rotation(45)
pyplot.savefig('obs_cluster/Binder ratio minv.jpg')
pyplot.clf()
...
if algorithm == "cluster":
    ...
    pyplot.legend()
    pyplot.savefig('obs_cluster/Binder ratio improved.jpg')
    pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Magnetic susceptibility.jpg')
# pyplot.show()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Energy Finite size scaling.jpg')
pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Specific heat Finite size scaling.jpg')
pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Squared magnetization Finite size scaling.jpg')
pyplot.clf()
...
if algorithm == "cluster":
    ...
    pyplot.legend()
    pyplot.savefig('obs_cluster/Squared magnetization improved Finite size scaling.jpg')
    pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Binder ratio Finite size scaling.jpg')
pyplot.clf()
...
if algorithm == "cluster":
    ...
    pyplot.legend()
    pyplot.savefig('obs_cluster/Binder ratio improved Finite size scaling.jpg')
    pyplot.clf()
...
pyplot.legend()
pyplot.savefig('obs_cluster/Magnetic susceptibility Finite size scaling.jpg')
pyplot.clf()
